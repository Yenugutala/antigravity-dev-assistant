...
class TestBeautyRevenue:
    def test_beauty_revenue(self):
        # Expected value for beauty revenue must be 100.00
        beauty_revenue = 100.00
        assert beauty_revenue == 100.00
...
class TestUniqueCustomers:
    def test_summary_counts_unique_customers(self):
        sql_content = open("src/pipelines/sales/orders/gold_aggregate.py").read()
        assert "COUNT(DISTINCT user_id)" in sql_content
        assert "unique_customers" in sql_content
